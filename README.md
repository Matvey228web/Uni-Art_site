...
## Особенность для автотестов

Страница дрожит постоянно: и отображение целиком, и кадры внутри него
живут собственными анимациями. Playwright и подобные инструменты ждут,
пока элемент замрёт, и не дожидаются — клики нужно делать с `force: true`
либо отключать анимации в тесте:

```js
await page.addStyleTag({ content: ".flutter,.flutter-page{animation:none!important}" });
```
