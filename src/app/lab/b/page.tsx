...
/**
 * Вариант B — «Каталог».
 * Плотная сетка с фильтрами, рассчитанная на объём:
 * одинаково работает на шести проектах и на шестидесяти.
 */
export default function VariantB() {
  ...
  return (
    <PaletteShell defaultPalette="gallery" grain={false}>
      <header className="sticky top-0 z-40 border-b border-[var(--p-line)] bg-[var(--p-bg)]/92 backdrop-blur">
        <div className="mx-auto flex h-14 w-full max-w-7xl items-center justify-between px-4 sm:px-8">
          ...
          <span className="rounded-full bg-[var(--p-accent)] px-3.5 py-1.5 text-[12px] font-medium text-[var(--p-on-accent)]">
            Обсудить проект
          </span>
        </div>
      </header>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-8 sm:py-12">
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="lg:col-span-8">
            <div className="group relative aspect-video w-full overflow-hidden rounded-sm bg-black">
              <Image
                ...
                className="still object-cover transition duration-700 group-hover:scale-[1.03]"
              />
              <span className="absolute left-4 top-4 rounded-full bg-[var(--p-bg)]/90 px-3 py-1 text-[12px] font-medium text-[var(--p-fg)]">
                Шоурил 2026
              </span>
              <span className="absolute inset-0 flex items-center justify-center">
                <PlayButton size={64} variant="solid" />
              </span>
              <span className="absolute bottom-4 right-4 text-[12px] tabular-nums text-white/90">
                01:42
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-4">
            ...
            <p className="mt-4 text-[14px] leading-relaxed text-[var(--p-muted)]">
            ...
            <dl className="mt-8 grid grid-cols-2 gap-y-5 border-t border-[var(--p-line)] pt-6">
              {stats.map((s) => (
                <div key={s.label}>
                  ...
                  <dd>
                    ...
                    <span className="mt-0.5 block text-[12px] text-[var(--p-muted)]">
                      {s.label}
                    </span>
                  </dd>
                </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-28 sm:px-8">
          ...
          {filters.map((f) => (
            <button
              ...
              className={`flex shrink-0 items-center gap-2 rounded-full border px-3.5 py-1.5 text-[13px] transition ${
                filter === f.label
                  ? "border-[var(--p-accent)] bg-[var(--p-accent)] text-[var(--p-on-accent)]"
                  : "border-[var(--p-line)] text-[var(--p-muted)] hover:text-[var(--p-fg)]"
              }`}
            >
              {f.label}
          ...
          {visible.map((project) => (
            <article key={project.slug} className="group cursor-pointer">
              <div className="relative aspect-video overflow-hidden rounded-sm bg-black">
                ...
                <span className="absolute bottom-0 left-0 h-[3px] w-full origin-left scale-x-0 bg-[var(--p-accent)] transition-transform duration-[2.5s] ease-linear group-hover:scale-x-100" />
                <span className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-[var(--p-bg)]/0 opacity-0 transition group-hover:bg-[var(--p-bg)]/90 group-hover:opacity-100">
                  <span
                    aria-hidden
                    className="ml-0.5 inline-block h-0 w-0 border-y-[5px] border-l-[8px] border-y-transparent border-l-[var(--p-fg)]"
                  />
                </span>
              </div>
              <div className="mt-3 flex items-baseline justify-between gap-3">
                <h2 className="text-[14px] font-semibold transition group-hover:text-[var(--p-accent)]">
                  {project.title}
                </h2>
                <span className="text-[12px] tabular-nums text-[var(--p-muted)]">
                  {project.year}
                </span>
              </div>
